...
class ProposalModel extends BaseModel {
  ...
  public function getProposal($page){
    $data = array();
    $sql  = 'SELECT nickName,avatarUrl,comment,create_time,z FROM __PREFIX__user INNER JOIN __PREFIX__proposal ON __PREFIX__user.id = __PREFIX__proposal.userid  ORDER BY z desc LIMIT 0,10';
    $rs 	= $this->query($sql);
    if($rs){
      $data['top'] = $rs;
